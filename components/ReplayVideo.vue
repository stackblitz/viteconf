...
const replayCurrentTime = useReplayCurrentTime();
...
onMounted(() => {
	...
	setInterval(() => {
		if (player && !done) {
			replayCurrentTime.value = player.value.getCurrentTime();
		}
	}, 1000);
});
</script>
...
<template>
	<div class="iframe-container">
		<div
			id="player"
			style="height: 100%; width: 100%; left: 0; position: absolute; top: 0"
			frameborder="0"
			loading="lazy"
			title="Viteconf livestream"
			allowfullscreen
